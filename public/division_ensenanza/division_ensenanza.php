...
$IMG_PLACEHOLDER = $ASSET_WEB . '/img/login.png';
...
<style>
  ...
  .image-panel{min-height:250px;display:grid;place-items:center;overflow:hidden;border-radius:16px;background:rgba(2,6,23,.55);border:1px solid rgba(148,163,184,.32);box-shadow:0 12px 28px rgba(0,0,0,.35)}
  .image-panel img{display:block;width:min(25%,180px);height:auto;object-fit:contain;padding:0;filter:drop-shadow(0 16px 30px rgba(0,0,0,.5))}
  .status-strip{margin-top:14px;padding:12px 14px;border-radius:14px;background:rgba(15,23,42,.82);border:1px solid rgba(148,163,184,.28);color:#cbd5f5;font-size:.88rem}
  @media (max-width:900px){.layout{grid-template-columns:1fr}.hero-inner{align-items:flex-start}.header-actions{margin-left:0}}
</style>
</head>
<body>
    ...
    <div class="layout">
      ...
      <section>
        <div class="image-panel">
          <img src="<?= e($IMG_PLACEHOLDER) ?>" alt="Division Ensenanza" onerror="this.onerror=null;this.src='<?= e($ESCUDO) ?>';">
        </div>
