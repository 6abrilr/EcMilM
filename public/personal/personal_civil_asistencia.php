...
function fmt_date_local(string $date): string {
    $dt = new DateTimeImmutable($date);
...
$exportScope = trim((string)($_GET['export_scope'] ?? 'week'));
$weekExport = trim((string)($_GET['semana'] ?? ''));
$dateExport = trim((string)($_GET['fecha_export'] ?? ''));
$weeklySummaryWeek = trim((string)($_GET['resumen_semana'] ?? ''));
$keepWeeklySummaryOpen = array_key_exists('resumen_semana', $_GET);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($accion === 'guardar_manual') {
            ...
            if (!isset($_FILES['archivo_excel']) || (int)$_FILES['archivo_excel']['error'] !== UPLOAD_ERR_OK) throw new RuntimeException(html_entity_decode('Seleccione un archivo v&aacute;lido para importar.', ENT_QUOTES, 'UTF-8'));
            $tmp = (string)$_FILES['archivo_excel']['tmp_name'];
            $name = (string)$_FILES['archivo_excel']['name'];
            $ext = strtolower((string)pathinfo($name, PATHINFO_EXTENSION));
            if (!in_array($ext, ['xlsx', 'xls', 'dat'], true)) throw new RuntimeException('El archivo debe ser .xlsx, .xls o .dat.');
            if ($accion === 'importar_padron') {
                $res = personal_civil_import_padron($pdo, $unidadPropia, $personalId, $tmp, $name);
                $flashOk = html_entity_decode('Padr&oacute;n civil importado. Registros procesados: ', ENT_QUOTES, 'UTF-8') . (int)$res['processed'] . '.';
            } else {
...
try {
    ...
    $todayLocal = (new DateTimeImmutable('now'))->format('Y-m-d');
    if ($selectedDate === '' && $dateFrom === '' && $dateTo === '') {
        if ($selectedYear === (int)date('Y')) {
            $selectedDate = $todayLocal;
        } elseif ($latestLoadedDate !== '') {
    ...
    if ($dateExport === '') {
        $dateExport = $selectedDate !== '' ? $selectedDate : ($latestLoadedDate !== '' ? $latestLoadedDate : $todayLocal);
    }
...
if (isset($_GET['export']) && $_GET['export'] === 'xls') {
    $exportScope = in_array($exportScope, ['week', 'day'], true) ? $exportScope : 'week';
    if ($exportScope === 'day') {
        if ($dateExport === '' || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $dateExport)) {
            http_response_code(400);
            exit('Falta el día para exportar.');
        }
        $rangeStart = $dateExport;
        $rangeEnd = $dateExport;
    } else {
        ...
        $rangeEnd = (new DateTimeImmutable($weekStart))->modify('+6 days')->format('Y-m-d');
    }
    ...
    $exportParams = [':u' => $unidadPropia, ':desde' => $rangeStart, ':hasta' => $rangeEnd];
    if (!$allPeopleExport) {
    ...
    $exportPeriod = $exportScope === 'day' ? $rangeStart : $rangeStart;
    header('Content-Type: application/vnd.ms-excel; charset=UTF-8');
    header('Content-Disposition: attachment; filename="personal_civil_' . $exportScope . '_' . $exportSuffix . '_' . $exportPeriod . '.xls"');
    echo "\xEF\xBB\xBF";
    $exportTitle = $exportScope === 'day'
        ? 'Planilla perteneciente al día ' . fmt_date_local($rangeStart)
        : 'Planilla de la semana desde el ' . fmt_date_local($rangeStart) . ' al ' . fmt_date_local($rangeEnd);
    echo '<html><head><meta charset="UTF-8"></head><body>';
    echo '<table border="1">';
    echo '<tr><th colspan="10">' . e($exportTitle) . '</th></tr>';
    echo '<tr><th>Fecha</th><th>DNI</th><th>Personal</th><th>Destino</th><th>Horario referencia</th><th>Ingreso</th><th>Egreso</th><th>Marcas</th><th>Horas</th><th>Observacion manual</th></tr>';
    foreach ($rows as $row) {
        echo '<tr>';
...
<header class="brand-hero">
  <div class="hero-inner container-main">
    ...
    <div class="header-back">
      <a href="personal.php" class="btn-ghost">Volver a Personal</a>
      <a href="personal_lista.php" class="btn-ghost">Ver lista de personal</a>
    </div>
  </div>
</header>
...
<div class="page-wrap">
  <div class="container-main">
    ...
    <div class="stats-grid">
      <div class="stat-card">
        <div class="stat-label">A&ntilde;o del resumen</div>
        <div class="stat-value"><?= e((string)$selectedYear) ?></div>
      </div>
      <div class="stat-card">
        <div class="stat-label">Civiles activos en padr&oacute;n</div>
        <div class="stat-value"><?= e((string)$activeCivilCount) ?></div>
      </div>
    </div>
    ...
    <div class="row g-3">
      <div class="col-lg-6">
        <div class="panel">
          <div class="panel-title">Importar padr&oacute;n civil</div>
          <div class="panel-sub">Se guarda en base de datos, tabla <code>personal_civil_padron</code>.</div>
          <form method="post" enctype="multipart/form-data" class="row g-2 align-items-end">
            <input type="hidden" name="accion_excel" value="importar_padron">
            ...
            <div class="col-12">
              ...
              <input type="file" name="archivo_excel" class="form-control" accept=".xlsx,.xls,.dat,text/plain" required>
            </div>
            <div class="col-12">
              <button type="submit" class="btn btn-success">Importar padr&oacute;n</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div class="col-lg-6">
        <div class="panel">
          <div class="panel-title">Importar registros de ingreso/egreso</div>
          <div class="panel-sub">Las marcas crudas se guardan en <code>personal_civil_registros</code>. Los ajustes manuales se guardan en <code>personal_civil_resumen_manual</code>.</div>
          <form method="post" enctype="multipart/form-data" class="row g-2 align-items-end">
            <input type="hidden" name="accion_excel" value="importar_registros">
            <input type="hidden" name="year" value="<?= e((string)$selectedYear) ?>">
            <input type="hidden" name="fecha" value="<?= e($selectedDate) ?>">
            <input type="hidden" name="desde" value="<?= e($dateFrom) ?>">
            <input type="hidden" name="hasta" value="<?= e($dateTo) ?>">
    ...
    <div class="panel">
      <div class="panel-title">Filtros del resumen</div>
      <form method="get" class="row g-2 align-items-end">
        <div class="col-md-2">
          <label class="form-label">A&ntilde;o</label>
          <select name="year" class="form-select">
            <?php foreach ($availableYears as $year): ?>
              <option value="<?= e((string)$year) ?>" <?= $year === $selectedYear ? 'selected' : '' ?>><?= e((string)$year) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="col-md-2">
          <label class="form-label">D&iacute;a</label>
          <input type="date" name="fecha" value="<?= e($selectedDate) ?>" class="form-control">
        </div>
        <div class="col-md-2">
          <label class="form-label">Desde</label>
          <input type="date" name="desde" value="<?= e($selectedDate === '' ? $dateFrom : '') ?>" class="form-control">
        </div>
        <div class="col-md-2">
          <label class="form-label">Hasta</label>
          <input type="date" name="hasta" value="<?= e($selectedDate === '' ? $dateTo : '') ?>" class="form-control">
        </div>
        <div class="col-md-3">
          <label class="form-label">Buscar</label>
          <input type="text" name="q" value="<?= e($q) ?>" class="form-control" placeholder="DNI, apellido o destino">
        </div>
        <div class="col-md-1 d-grid">
          <button type="submit" class="btn btn-outline-light">Aplicar</button>
        </div>
      </form>
      <div class="small-muted mt-2">&Uacute;ltimo d&iacute;a cargado en <?= e((string)$selectedYear) ?>: <strong><?= e($latestLoadedDate !== '' ? $latestLoadedDate : 'sin datos') ?></strong></div>
    </div>

    <div class="row g-3">
      <div class="col-12">
        <div class="panel">
          <div class="panel-title">Resumen diario <?= e((string)$selectedYear) ?></div>
          <div class="panel-sub">Si eleg&iacute;s un d&iacute;a, la tabla muestra solamente ese d&iacute;a. Si dej&aacute;s el d&iacute;a vac&iacute;o y carg&aacute;s desde/hasta, te muestra el rango.</div>
            ...
            <table class="table table-dark table-hover align-middle">
              <thead>
                <tr>
                  ...
                  <th>Marcas</th>
                  <th>Horas</th>
                  <?php if ($esAdmin): ?><th>Ajuste manual</th><?php endif; ?>
                </tr>
              </thead>
              <tbody>
                <?php if ($summaryRows === []): ?>
                  <tr>
                    <td colspan="<?= $esAdmin ? '10' : '9' ?>" class="text-center text-secondary">No hay registros para el filtro seleccionado.</td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($summaryRows as $row): ?>
                    <tr>
                      <td><?= e((string)$row['fecha']) ?></td>
                      <td><?= e((string)$row['dni']) ?></td>
                      <td><?= e((string)$row['persona']) ?></td>
                      <td><?= e((string)$row['destino_interno']) ?></td>
                      <td><span class="pill"><?= e((string)$row['horario_referencia']) ?></span></td>
                      <td><?= e(fmt_time((string)$row['ingreso'])) ?></td>
                      <td><?= e(fmt_time((string)$row['egreso'])) ?></td>
                      <td><?= e((string)$row['marcas']) ?></td>
    ...
    <div class="row g-3">
      <div class="col-12">
        <details class="panel">
          <summary class="panel-title">Exportar por semana y persona</summary>
          <div class="panel-sub">Pod&eacute;s exportar toda la semana o un solo d&iacute;a, para todo el personal civil o para una persona puntual, en formato Excel (.xls).</div>
          <form method="get" class="row g-2 align-items-end">
            <input type="hidden" name="export" value="xls">
            <input type="hidden" name="year" value="<?= e((string)$selectedYear) ?>">
            <div class="col-md-6">
              <label class="form-label">Persona</label>
              <select name="persona_dni" class="form-select">
                <option value="__ALL__" <?= ($personExport === '__ALL__' || $personExport === '') ? 'selected' : '' ?>>Todo el personal civil</option>
                <?php foreach ($padron as $row): ?>
                  <option value="<?= e((string)$row['dni']) ?>" <?= $personExport === (string)$row['dni'] ? 'selected' : '' ?>><?= e((string)$row['apellido_nombre']) ?> &middot; <?= e((string)$row['dni']) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="col-md-4">
              <label class="form-label">Semana</label>
              <input type="week" name="semana" value="<?= e($weekExport) ?>" class="form-control" required>
            </div>
            <div class="col-md-4">
              <label class="form-label">D&iacute;a</label>
              <input type="date" name="fecha_export" value="<?= e($dateExport) ?>" class="form-control">
            </div>
            <div class="col-md-1 d-grid">
              <button type="submit" name="export_scope" value="week" class="btn btn-outline-success">Semana</button>
            </div>
            <div class="col-md-1 d-grid">
              <button type="submit" name="export_scope" value="day" class="btn btn-outline-info">D&iacute;a</button>
            </div>
          </form>
        </details>
      </div>
      ...
      <div class="col-12">
        <details class="panel" id="horas-por-persona" <?= $keepWeeklySummaryOpen ? 'open' : '' ?>>
          <summary class="panel-title">Horas por persona</summary>
          <div class="panel-sub">Resumen semanal desplegable. Eleg&iacute; la semana y te muestra d&iacute;as con marca y horas trabajadas por cada persona.</div>
          <form method="get" action="#horas-por-persona" class="row g-2 align-items-end mb-3">
            <input type="hidden" name="year" value="<?= e((string)$selectedYear) ?>">
            <input type="hidden" name="fecha" value="<?= e($selectedDate) ?>">
            <input type="hidden" name="desde" value="<?= e($selectedDate === '' ? $dateFrom : '') ?>">
            <input type="hidden" name="hasta" value="<?= e($selectedDate === '' ? $dateTo : '') ?>">
            <input type="hidden" name="q" value="<?= e($q) ?>">
            <div class="col-md-4">
              <label class="form-label">Semana a resumir</label>
              <input type="week" name="resumen_semana" value="<?= e($weeklySummaryWeek) ?>" class="form-control">
            </div>
            <div class="col-md-2 d-grid">
              <button type="submit" class="btn btn-outline-light">Ver resumen semanal</button>
            </div>
          </form>
          <div class="table-responsive">
            <table class="table table-dark align-middle">
              ...
              <tbody>
                <?php if ($personTotals === []): ?>
                  <tr>
                    <td colspan="3" class="text-center text-secondary">Sin datos para la semana elegida.</td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($personTotals as $row): ?>
                    <tr>
                      <td>
                        <div><?= e((string)$row['persona']) ?></div>
                        <div class="small-muted"><?= e((string)$row['dni']) ?></div>
                      </td>
                      <td><?= e((string)$row['dias_con_marca']) ?></td>
                      <td><strong><?= e(fmt_hours((int)$row['segundos_periodo'])) ?></strong></td>
                    </tr>
                  <?php endforeach; ?>
                <?php endif; ?>
              </tbody>
      ...
      <div class="col-12">
        <details class="panel">
          <summary class="panel-title">Padr&oacute;n civil activo</summary>
          <div class="panel-sub">DNI sin padr&oacute;n activo detectados en <?= e((string)$selectedYear) ?>: <strong><?= e((string)$unmatchedCount) ?></strong></div>
          <div class="table-responsive">
            <table class="table table-dark align-middle">
              <thead>
                <tr>
              ...
              <tbody>
                <?php if ($padron === []): ?>
                  <tr>
                    <td colspan="3" class="text-center text-secondary">Todav&iacute;a no hay padr&oacute;n civil importado.</td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($padron as $row): ?>
                    <tr>
                      <td><?= e((string)$row['dni']) ?></td>
                      <td>
                        <div><?= e((string)$row['apellido_nombre']) ?></div>
                        <div class="small-muted"><?= e((string)$row['horario_referencia']) ?></div>
                      </td>
                      <td><?= e((string)$row['destino_interno']) ?></td>
                    </tr>
                  <?php endforeach; ?>
